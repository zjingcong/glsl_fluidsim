...
vec4 bilinear(sampler2D mat, vec2 pos, vec2 resolution)
{
    ...
    vec4 q11 = texture2D(mat, boundary(ij)/resolution);
    vec4 q21 = texture2D(mat, boundary(ij + vec2(1.0, 0.0))/resolution);
    vec4 q12 = texture2D(mat, boundary(ij + vec2(0.0, 1.0))/resolution);
    vec4 q22 = texture2D(mat, boundary(ij + vec2(1.0, 1.0))/resolution);
...
void main()
{
    ...
    // vec2 velocity = texture2D(u_velocity, pos0/u_resolution).xy;
    vec2 velocity = bilinear(u_velocity, pos0, u_resolution).xy;

    vec2 pos = fragCoord - pixel - velocity * u_dt;
    pos = boundary(pos);

    // vec4 color = texture2D(u_color, pos/u_resolution);
